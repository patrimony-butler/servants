...
impl ConfigLoader for ConfigData {
    fn load(config_name: ConfigName) -> ServantResult<Box<ConfigData>> {
        ...
        let port = ConfigReader::read_port(&config, "default", "port", 6100_u16)?;
        let host = ConfigReader::read_host(&config, "default", "host", "0.0.0.0")?;
        let addr = SocketAddrV4::new(host, port);

        let butler_port = ConfigReader::read_port(&config, "default", "butler_port", 6000_u16)?;
        let butler_host = ConfigReader::read_host(&config, "default", "butler_host", "0.0.0.0")?;
